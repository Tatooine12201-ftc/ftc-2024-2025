    ...
    public void runOpMode() {
        ...
        Actions.runBlocking(new ParallelAction(
                ...
                new ParallelAction(
                        ...
                        , arm.setExtend(14.3 , 4.0)
                        , goScoreSpecimen.build())
                , new SequentialAction(
                        new ParallelAction(
                new InstantAction(()-> arm.setAngle(46, 2.5)))
                , arm.setExtend(0, 4)
                )
                ...
                , new ParallelAction(arm.setAngle(-1, 2.5), arm.setExtend((arm.getMaxExtend() - 4) , 4.0)
                , new ParallelAction(goScoreSample1.build(), arm.setAngle(70, 2.5))
                , intake.intakeByTimer(-0.25, 0.5)
                , arm.setAngle(66 , 2.5)
                , intake.intakeByTimer(1, 0.01)
                , goIntakeSample2.build()
                , new ParallelAction(arm.setExtend(arm.getMaxExtend() - 30 , 4.0)  , new SequentialAction(
                        arm.setAngle(30, 2.5)))
                ,arm.setAngle(-1, 2.5)
                ,arm.setExtend(arm.getMaxExtend() - 12.0, 4.0)
                , new SleepAction(1)
                , arm.setExtend(arm.getMaxExtend() - 10.0, 4.0)
                ,  new SleepAction(1)
                , new InstantAction(wrist::intakeFlat)
                , new InstantAction(wrist::intakeFlat)
                , arm.setAngle(40, 2.5)
                , new ParallelAction( new InstantAction(wrist::intakeFlat), goScoreSample2.build(), arm.setAngle(70, 2.5),  new InstantAction(wrist::intakeFlat), arm.setExtend(arm.getMaxExtend()-4 , 4.0 ))
                , intake.intakeByTimer(-0.25, 0.5)
                , intake.intakeByTimer(1, 0.01)
                ...
                , new ParallelAction(arm.setAngle(Conts.angleScoreSample, 2.5), arm.setExtend(arm.getMaxExtend() - 0.5, 4.0))
                , new InstantAction(wrist::doc),new SleepAction(0.5), intake.intakeByTimer(-0.25, 0.5)
                , new SequentialAction(intake.stop(),new InstantAction(wrist::intakeFlat), arm.setExtend(0, 4.0), arm.setAngle(Conts.angleDrive, 2.5))
        )
        )));
    }
